...
def unpack(real_archive_path: str, normalized_archive_path: str) -> None:
  """
  ...
  if (ext_index := normalized_archive_path.rfind('.')) != -1:
    ...
    return (False, CONST.ERROR_NO_EXTENSION)
...
def move_file_to_group_dir(group_name: str, current_path: str, new_path: str) -> tuple:
  ...
  - instead of moving an archive function unpacks it and the unpacked content will be placed to a directory with the name of the archive without an extension and this directory in order will be placed in a group directory
  """
...
def get_file_list(directory: str, root_directory="", recursion_depth=0) -> dict:
  """
  - recursive function which scans the given directory and forms the list of grouped files
  ...
  - instead of resulting path "new_path" key can contain an error description (for example if we're trying to replace an existing file or directory)
  """
  result = {}
  deeper_result = {}
  ...
  if directory:
    ...
    if os.path.islink(directory):
      ...
      print("To treat the symlink as a directory try to add a path separator at the end of the path.")
      return {}
